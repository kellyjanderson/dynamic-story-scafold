---
base: main
branch: feature/mvp-09-persistence
capabilities:
  mcp:
    optional:
    - codex_app
...
completion:
  ...
  - branch-diff-check
dependencies:
- mvp-08-atomic-round-commit
id: mvp-09-persistence
legacy_source: project/WorkOrders/mvp-09-persistence.md
...
scope:
  ...
  owns:
  - '**'
title: MVP-09 — Minimal simulation persistence and idempotent round commit
---

# Codex-native execution overlay

This Work Order was converted from `project/WorkOrders/mvp-09-persistence.md`. This overlay overrides any
...
Use `unit` and `integration`; before PR also request `the relevant project-native check` and `the relevant project-native check`. Controller-required targets must remain green. Then PR/merge/WORK_COMPLETE.
