---
base: main
branch: feature/mvp-03-dependencies-arbitration
capabilities:
  mcp:
    optional:
    - codex_app
...
completion:
  ...
  - branch-diff-check
dependencies:
- mvp-02-round-coordinator
id: mvp-03-dependencies-arbitration
legacy_source: project/WorkOrders/mvp-03-dependencies-arbitration.md
...
scope:
  ...
  owns:
  - '**'
title: MVP-03 — Claims, dependency graph, and bounded arbitration
---

# Codex-native execution overlay

This Work Order was converted from `project/WorkOrders/mvp-03-dependencies-arbitration.md`. This overlay overrides any
...
Use `unit` frequently; use `integration` for coordinator + graph arbitration. Final required targets plus `the relevant project-native check` if property tests materially alter broad behavior. Then PR/merge/WORK_COMPLETE.
